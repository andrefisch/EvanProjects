...
def formatField(string):
    return string.replace('&amp;', '&').replace('\\', '').replace('xe', '').replace('2x80x99', "'").strip()
...
def scrapeWista():
    ...
    DONTTITLECASE = ['EMAIL', 'WEBSITE', 'JOBDESCRIPTION', 'DESCRIPTIONOFCOMPANY', 'LASTUPDATE']
    ...
    for j in range(0, len(urls)):
        ...
        for i in range(0, len(info)):
            if info[i].upper().replace(' ', '').replace('-', '').strip() in dicty:
                if current != "":
                    newInfo.append(formatField(current))
        ...
        while(True):
            ...
            for i in range(0, len(newInfo)):
                if last and newInfo[i] and newInfo[i].upper().replace(' ', '').replace('-', '').strip() in dicty:
                    del newInfo[i - 1]
                    change = True
                    last = False
                    break
        ...
        for i in range(0, len(newInfo), 2):
            ...
            if column not in DONTTITLECASE:
                inf = inf.title()
